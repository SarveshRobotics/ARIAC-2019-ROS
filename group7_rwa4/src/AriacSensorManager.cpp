...
//        ROS_INFO_STREAM("[ASM]:[grab_gear]: inside the for loop");
        ...
        try{
            ...
            ROS_INFO_STREAM("[ASM]:[grab_gear]: inside the try ");

            geometry_msgs::Pose part_pose = msg;

            ROS_INFO_STREAM("[ASM]:[grab_gear]: arm1 busy flag check  " << int(!arm1_busy));
            ROS_INFO_STREAM("[ASM]:[grab_gear]: desired parts info check " <<  int(desired_parts_info.find("gear_part") != desired_parts_info.end()));

            if (!arm1_busy && (desired_parts_info.find("gear_part") != desired_parts_info.end())){
                arm1_busy = true;
                arm1.SendRobotTo(arm1_bin_pose);
                arm1.SendRobotTo("linear_arm_actuator_joint",-1);
